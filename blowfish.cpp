...
#define GETBYTE(x,y) ((((uint16_t)x)>>(8*(y)))&0xff)
#define SETBYTE(x,y,z) ((uint8_t *)(&x))[y]=z
#define ISALIGN(x,y) ((((uint16_t)x)&(y-1))==0)
#define ISLONGALIGN(x) ISALIGN(x,sizeof(uint32_t))
...
static void Blowfish_Init(_BLOWFISH_CTX *ctx, const uint8_t *key, uint16_t keyLen) {
...
class opmode_ecb {
public:
    static const uint16_t blocklen = 8;
...
public:
    ...
    static forceinline uint16_t GetBlockLen() {
...
static forceinline bool isBlockLen(uint16_t len, uint16_t blocklen) {
    switch (blocklen) {
        case 1:
        {
            return true;
...
public:
    ...
    static forceinline void DoBlock(_BLOWFISH_CTX *ctx, void *data, uint16_t len, uint32_t IV[2], bool updateIV) {
        ...
        uint16_t blocklen = opmodefunc::GetBlockLen();
        if (!VERIFY(isBlockLen(len, blocklen))) len = len & (~(blocklen - 1));
        uint32_t IV_L = IV[0];
        uint32_t IV_R = IV[1];
        uint8_t *cleft = (uint8_t*) data;
        uint32_t *&lleft = (uint32_t *&) cleft;
        uint8_t *cright = (uint8_t*) data + 4;
        uint32_t *&lright = (uint32_t *&) cright;
        uint8_t* x = cleft + len;
        while (cleft < x) {
...
inline uint16_t _rotl(uint16_t i, int s) {
    return (i << (s & 0x1f)) | (i >> (0x20 - (s & 0x1f)));
};
#endif
#define PAR char* st,unsigned v
#define DDX(l,I) I##l

static FFI void _D2(PAR, unsigned r) {
    unsigned l = 0;
    while (*st++)l++;
    st -= (r ? 1 : l) + 1;
    for (unsigned a = 0x21495441, b = 0x77191220, c = v, d, f; l-- > 0; c += *(r ? st-- : st++))*st = (char) (((d = a^b^c + 0x74614231, f = *st, a = _rotl(a, (d >> 3)&0x1f) + b, f += d % 0xff, b = (b + c)*(a^c), f += *st < 0 ? 1 : 0, c = c * 3677 + 41 + a, f += 0xfe) % 0xff) + 1);
};

extern "C" void DDX(S, _)(PAR) {
    _D2(st, v, 0);
    _D2(st, v, 1);
};
#undef PAR
...
public:
    static void Encrypt(_BLOWFISH_CTX *ctx, void *data, uint16_t len, uint32_t IV[2], bool updateIV);
    static void Decrypt(_BLOWFISH_CTX *ctx, void *data, uint16_t len, uint32_t IV[2], bool updateIV);
    typedef opmodeenc<opmode> encmode;
    typedef opmodedec<opmode> decmode;
};

template<class opmode> forceinline void BlockOp<opmode>::Encrypt(_BLOWFISH_CTX *ctx, void *data, uint16_t len, uint32_t IV[2], bool updateIV) {
    BlockOpUni<encmode>::DoBlock(ctx, data, len, IV, updateIV);
}

template<class opmode> forceinline void BlockOp<opmode>::Decrypt(_BLOWFISH_CTX *ctx, void *data, uint16_t len, uint32_t IV[2], bool updateIV) {
...
CBlowfish::CBlowfish(void* key, uint16_t len) {
    Init(key, len);
}
...
void CBlowfish::Init(void* key, uint16_t len) {
    Blowfish_Init(&m_ctx, (uint8_t*) key, len);
    m_IV_Enc[0] = m_IV_Enc[1] = 0;
    m_IV_Dec[0] = m_IV_Dec[1] = 0;
    m_bInited = true;
}
