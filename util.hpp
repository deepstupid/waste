...
//crc
uint32_t crc32(uint32_t crc, unsigned char *buf, uint16_t len);
...
template <class T,class U> void inline TIntData4(U data,T i)
{
	...
	data2[0]=(unsigned char)(((uint16_t)i>> 0)&0xff);
	data2[1]=(unsigned char)(((uint16_t)i>> 8)&0xff);
	data2[2]=(unsigned char)(((uint16_t)i>>16)&0xff);
	data2[3]=(unsigned char)(((uint16_t)i>>24)&0xff);
};
...
{
	unsigned char*& data2=(unsigned char*&)data;
	data2[0]=(unsigned char)(((uint16_t)i>>24)&0xff);
	data2[1]=(unsigned char)(((uint16_t)i>>16)&0xff);
	data2[2]=(unsigned char)(((uint16_t)i>> 8)&0xff);
	data2[3]=(unsigned char)(((uint16_t)i>> 0)&0xff);
};

template <class T,class U> void inline TIntDataSwap2(U data,T i)
{
	unsigned char*& data2=(unsigned char*&)data;
	data2[0]=(unsigned char)(((uint16_t)i>> 8)&0xff);
	data2[1]=(unsigned char)(((uint16_t)i>> 0)&0xff);
};
...
uint32_t   inline Datauint32_t	   (unsigned char* c) {return TDataInt4	   <uint16_t,  unsigned	char*>(c);};
uint32_t   inline DataUIntSwap4(unsigned char* c) {return TDataIntSwap4<uint16_t,  unsigned	char*>(c);};
unsigned short inline Datauint16_t	   (unsigned char* c) {return TDataInt2	   <unsigned short,unsigned	char*>(c);};
...
void inline	UIntData4	 (unsigned char* c,unsigned	int	  i) {TIntData4	   <uint16_t  ,unsigned	char*>(c,i);};
