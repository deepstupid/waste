...
public:
	...
	void set_dc_ipport(uint32_t ip, unsigned short port)
	{
		m_ip=ip;
		m_port=port;
	};
	void get_dc_ipport(uint32_t *ip, unsigned short *port)
	{
		*port=m_port;
		*ip=m_ip;
	};
	...
	void add_need_chunk(uint16_t idx);

	int get_chunks_needed() { return m_need_chunks_used; }
	uint16_t get_need_chunk(int idx) //only valid on the decompress side (otherwise returns RLE crap)
	{
...
protected:
	...
	uint16_t m_need_chunks_offs[FILE_MAX_CHUNKS_PER_REQ];
...
public:
	...
	void get_file_len(uint16_t *low, uint16_t *high) { *low = m_file_len_low; *high=m_file_len_high; }
	void set_file_len(uint16_t file_len_low, uint16_t file_len_high);

	void set_file_dates(uint16_t create_date, uint16_t mod_date) { m_create_date=create_date; m_mod_date = mod_date; }
	void get_file_dates(uint16_t *create_date, uint16_t *mod_date) { *create_date=m_create_date; *mod_date=m_mod_date; }
...
protected:
	...
	uint16_t m_file_len_low, m_file_len_high;
	uint16_t m_create_date;
	uint16_t m_mod_date;
	uint16_t m_chunkcnt;
	uint32_t m_ip;
	unsigned short m_port;
